...
primitive SVG
  ...
  fun group(children: Iterator[SVGNode],
    stroke: String = "black", stroke_width: String = "1"): SVGNode
  =>
    let node = SVGNode("g")
      .>a("stroke", stroke)
...
class PathCommands
  ...
  fun ref command(c: PathCommand) =>
    match c
    | let c': PathMove if _command_list.size() > 0 =>
      try
        match _command_list(_command_list.size() - 1)?
        | let lc': PathMove =>
          _command_list(_command_list.size() - 1)? = c
        else
  ...
  fun ref commands(cs: PathCommands) =>
    for c in cs._command_list.values() do
      command(c)
    end
