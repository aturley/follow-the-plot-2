...
class val Segment
  ...
  fun val intersection(that: Segment): (Point | None) =>
    ...
    | (let m1: InfSlope, let m2: InfSlope, None, None) if m1.y == m2.y =>
    ...
    | (let m1: InfSlope, let m2: F64, None, let b2: F64) =>
      ...
      let y = m1.y
      let p = Point(x, y)
      if p.in_rect(that.p1, that.p2) then
        p
      end
...
class Line
  ...
  fun ref add(p: Point) =>
    try
      ...
      _segments.push(Segment(_start_point, p))
    end

  fun ref close() =>
    try
      _segments.push(Segment(_segments(_segments.size() - 1)?.p2, _start_point))
    end
